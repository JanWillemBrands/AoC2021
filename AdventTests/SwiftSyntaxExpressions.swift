...
let expressionSnippets: [SwiftSnippet] = [
    ...
    SwiftSnippet(label: "testKeyPathFollowedByOperator#1", source: #"\Foo?.?.bar.?.blah"#, origin: "ExpressionTests.testKeyPathFollowedByOperator", syntaxVersion: "603.0.1", disabledReason: "valid Swift — swift-syntax accepts cleanly (hasError=false): keypath `\\Foo?.?.bar` + infix `binaryOperator \".?.\"` + `blah` (token-probe confirmed; the old 'error recovery' reason was wrong). Advent can't accept yet: the eager single-token scanner can't offer `.?.` (operator) and `.`/`?` (keypath split) as competing tokenizations — needs multi-lex. Same construct as #2 (which stays enabled as the live tracker)."),
...
@Suite("SwiftSyntax - Expressions — SwiftSyntax comparison")
