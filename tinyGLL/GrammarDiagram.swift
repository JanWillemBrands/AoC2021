        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                if let selected = selectedNode {
                    ...
                    section("Node") {
                        row("symbol", node.kind == .ALT || node.kind == .END
                                      ? "—" : String(node.name))
                        row("kind", "\(node.kind)")
                        row("number", "#\(node.number)")
                        row("cluster", String(selected.cluster))
                        row("grid", "row \(Int(selected.y)), col \(Int(selected.x))")
                    }

                    section("Links") {
                        row("seq", node.seq.map { "#\($0.number)  \($0.kind)" } ?? "none")
                        row("alt", node.alt.map { "#\($0.number)  \($0.kind)" } ?? "none")
