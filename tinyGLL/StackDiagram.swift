...
///
/// Column widths are measured from the labels rather than fixed, because a slot box is
/// several times wider than an `(X, k)` oval and a uniform grid would either overlap the
/// boxes or strand the ovals.
struct CRFLayout {
    ...
    init(snapshot: CRFSnapshot) {
        ...
        for cluster in snapshot.clusters {
            ...
            nodes.append(Node(id: id,
                              ...
                              row: rowOf[id] ?? 0,
                              size: CGSize(width: Self.width(label, font: Self.clusterFont) + 26, height: 28),
        ...
        for returnNode in orderedReturns {
            ...
            nodes.append(Node(id: id,
                              ...
                              row: rowOf[id] ?? 0,
                              size: CGSize(width: Self.width(label, font: Self.returnFont) + 22, height: 26),
                              isRoot: false,
    ...
    private static let clusterFont = NSFont.monospacedSystemFont(ofSize: 12, weight: .semibold)
    private static let returnFont = NSFont.monospacedSystemFont(ofSize: 11, weight: .regular)
...
struct StackDiagramView: View {
    ...
    private var controls: some View {
        HStack(spacing: 4) {
            ...
            Slider(value: $zoom, in: 0.6...1.8).frame(width: 90)
    ...
    private func clusterDetail(_ node: CRFLayout.Node) -> some View {
        ...
        section("Cluster node") {
            ...
            row("entered at", "\(node.position.index)")
            row("production", node.position.slot.production)
            row("children", "\(cluster?.returns.count ?? 0)")
            // The paper's contingent return set P, which for this node is the set of input
            // positions the call has been seen to reach.
            row("returns at", (cluster?.pops ?? []).map(String.init).joined(separator: ", "))
            if node.isRoot {
    ...
    private func returnDetail(_ node: CRFLayout.Node) -> some View {
        section("Return node") {
            row("slot", CRFLabels.slot(after: node.position.slot))
            row("called", String(node.position.slot.name))
            row("frame from", "\(node.position.index)")
            row("shared by", "\(node.ownerCount) cluster\(node.ownerCount == 1 ? "" : "s")")
        }
    ...
    private var summary: some View {
        section("Call return forest") {
            row("clusters", "\(snapshot.clusters.count)")
            row("return nodes", "\(snapshot.returnNodeCount)")
            row("edges", "\(snapshot.edgeCount)")
        }
...
private struct CRFNodeBadge: View {
    ...
    var body: some View {
        Text(node.label)
            .font(.system(size: node.kind == .cluster ? 12 * zoom : 11 * zoom,
                          weight: node.kind == .cluster ? .semibold : .regular).monospaced())
            .padding(.horizontal, 9 * zoom)
            .padding(.vertical, 4 * zoom)
            .background {
                // Layered over the edges, so an opaque backdrop under the fill keeps the
                // lines from running through the label.
                shape.fill(Color(nsColor: .textBackgroundColor))
                shape.fill(fill)
            }
            .overlay(shape.stroke(stroke, lineWidth: isSelected ? 2.5 : (node.isRoot ? 2 : 1)))
